...
import { 
  ...
  ShoppingCart, ScanFace, Users, BarChart3, Settings
} from 'lucide-react';
...
export default function AdminSidebar() {
  ...
  const alertsCount = useMemo(() => {
    ...
    inventory.forEach(m => {
      // Skip if reviewed
      if (Array.isArray(reviewedAlerts) && reviewedAlerts.includes(m.id)) return;

      const expDate = new Date(m.expiryDate);
      const daysDiff = (expDate.getTime() - today.getTime()) / (1000 * 3600 * 24);
      
      const isLowStock = m.stock < 10;
      const isExpiring = daysDiff <= 7;
      const needsStorageAlert = m.isPerishable;
      
      if (isLowStock || isExpiring || needsStorageAlert) {
        count++;
      }
    });
    return count;
