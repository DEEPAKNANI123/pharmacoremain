...
export default function PatientDashboard() {
  ...
  const recommendedMeds = inventory.slice(0, 6);
          ...
          messages: [
            {
              ...
              content: [
                {
                  type: "text",
                  text: "You are an automated pharmacy assistant. Please analyze this prescription image and return ONLY a valid JSON object. Extract 'patientName', 'doctorDetails', 'hospitalDetails' and an array of 'medicines'. For each medicine, provide: 'name', 'prescribedDoseTiming' (the instruction from the script), 'conditionTreated' (what it is for), and 'standardDose' (the typical generic dose for this medicine). Format: { \"patientName\": \"...\", \"doctorDetails\": \"...\", \"hospitalDetails\": \"...\", \"medicines\": [ { \"name\": \"...\", \"prescribedDoseTiming\": \"...\", \"conditionTreated\": \"...\", \"standardDose\": \"...\" } ] }"
                },
                {
                  type: "image_url",
                  image_url: {
        ...
        <div className="rx-upload-card">
          <div className="rx-text">
            ...
            <div className="flex gap-4">
              ...
              />
              <button className="btn btn-white px-10 py-5 flex-center gap-3 text-lg font-bold shadow-xl" onClick={() => fileInputRef.current?.click()} disabled={isUploading}>
                {isUploading ? <><Loader2 size={24} className="spinner" /> Analyzing...</> : <><Camera size={24} /> Start Scan</>}
              </button>
              <button className="btn btn-glass px-10 py-5 flex-center gap-3 text-lg font-bold" onClick={() => fileInputRef.current?.click()} disabled={isUploading}>
                 <ImageIcon size={24} /> From Gallery
              </button>
            </div>
