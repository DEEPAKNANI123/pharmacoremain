...
export default function Alerts() {
  const { inventory, markAlertAsReviewed, markAllAlertsAsReviewed, reviewedAlerts } = useDatabase();
  ...
  const alertsList = useMemo(() => {
    ...
    return list.filter(a => !reviewedAlerts.includes(a.medId) || a.id === 'alert-comp');
          ...
          <div className="flex-gap">
            {alertsList.length > 0 && alertsList[0].id !== 'alert-comp' && (
              <button 
                ...
                onClick={() => markAllAlertsAsReviewed(alertsList.map(a => a.medId).filter(Boolean))}
              >
          ...
          {alertsList.map(a => (
            <div key={a.id} className="alert-item" style={{ 
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <button className={`btn btn-sm ${a.type === 'Low Stock' || a.type === 'Critical Stock' ? 'btn-primary' : 'btn-outline'}`} style={{ width: '100%' }}>
                ...
                {a.medId && (
                  <button 
                    ...
                    onClick={() => markAlertAsReviewed(a.medId)}
                  >
                    Dismiss
                  </button>
                )}
